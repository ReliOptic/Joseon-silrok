...
interface Level1Props {
  readonly setActiveEra: (era: Era) => void;
...
function getSuccessionStyle(succession: KingListItem['succession']): SuccessionStyle {
  switch (succession) {
    case 'coup':           return { borderLeft: 'border-l-2 border-l-red-300/50', leftBadge: '⚔' };
    case 'short_lived':    return { opacity: 'opacity-70', bg: 'bg-white/10' };
    case 'usurpation':     return { borderLeft: 'border-l-2 border-dashed border-l-amber-400/60' };
    case 'abdication':     return { rightBadge: '↓' };
    case 'forced_abdication': return { rightBadge: '⊗' };
    case 'enthronement':   return { leftBadge: '★' };
    default:               return {};
  }
}

interface KingCardProps {
  readonly king: KingListItem;
  readonly onSelect: (id: string) => void;
}

function KingCard({ king, onSelect }: KingCardProps){
    ...
    >
      ...
      >
        <div className="flex items-baseline gap-4 mb-1">
          {sStyle.leftBadge && <span className="text-xs opacity-50 shrink-0">{sStyle.leftBadge}</span>}
          ...
          {sStyle.rightBadge && <span className="text-xs opacity-40 shrink-0">{sStyle.rightBadge}</span>}
        ...
        {KING_HOOKLINES[king.id] && (
          <p className="text-base font-medium leading-[1.5] tracking-[-0.01em] mb-2">{KING_HOOKLINES[king.id]}</p>
        )}
            ...
            <div className="pl-12">
              ...
              <div className="flex flex-col gap-4 relative">
                ...
                {era.kingsList.map(king => (
                  <KingCard key={king.id} king={king} onSelect={onSelectKing} />
                ))}
              </div>
            </div>
          </div>
